...
[![version](https://img.shields.io/badge/version-2.6.0-7c6aff)](#)
...
| 機能 | 説明 |
|---|---|
| **タスク管理** | プロジェクト・サブタスク・カンバン・アイゼンハワーマトリックス |
| **タイムトラッキング** | 作業時間の計測・過集中防止リマインダー |
| **保留・待機** | ⏸ 保留（一時停止）/ ≈ 待機（返事待ちなど長期）・経過日数の可視化 |
| **カンバン連動** | タスク開始→実行中・保留→保留中・待機→待機中・完了→完了に自動連動 |
| **アーカイブ** | 月またぎで完了タスクを自動アーカイブ・18ヶ月後に自動削除 |
| **タスク検索** | キーワード検索・完了済み含む検索 |
...
|---|---|
| 未着手 | まだ手をつけていないタスク |
| 実行中 | 計測中のタスク（タイマーが動いている） |
| 保留中 | 一時停止（数分〜1時間程度） |
| 待機中 | 長期待機（返事待ち・承認待ちなど）・経過日数を表示 |
| 完了 | 完了したタスク |
